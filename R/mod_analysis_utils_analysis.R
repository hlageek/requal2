...
load_segments_analysis <- function(
  pool,
  active_project,
  selected_codes,
  selected_categories,
  ...
  if (isTruthy(selected_codes) | isTruthy(selected_categories)) {
    ...
    code_filter <- as.integer(unique(c(
      category_edges[isTruthy(category_edges)],
...
format_segments <- function(
  ns,
  segment_id,
  segment_text,
  segment_document_id,
  segment_document_name,
  segment_code,
  segment_color
) {
  tags$div(
    ...
    tags$div(
      ...
      segment_code %>%
        tags$div(
          class = "segment_badge",
          style = paste0("background-color: ", segment_color, " !important;")
        ),
